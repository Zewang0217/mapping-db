...
DIMENSION_VALUES = [
    ...
    ("mech", "代码执行", "直接执行恶意代码/命令——exec/eval/subprocess、命令注入、RCE；扫描器检测锚点"),
    ("mech", "权限滥用", "滥用超出所需的权限/能力——权限提升、权限请求、越权调用工具"),
    ("mech", "方式不明", "原始分类未陈述具体攻击手段"),
    # 暂不预置（未遇到）：依赖操纵 — 映射中遇到再讨论加回
    ...
    # Carrier tags (through which medium the attack data flows; like vuln tags, optional)
    ("carrier", "提示词", "攻击经文本/指令载体实施——注入、覆盖、越狱指令"),
    ("carrier", "代码", "攻击经代码载体实施——脚本、程序逻辑、命令执行"),
    ...
    ("carrier", "载体不明", "原始分类未陈述攻击通过的媒介；声明/配置类攻击无数据流可不填"),
]
