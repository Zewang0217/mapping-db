...
DIMENSION_VALUES = [
    ...
    ("mech", "依赖投毒", "篡改依赖版本、引用恶意仓库、利用已知 CVE 的版本"),
    ...
    # Vulnerability tags (8 values)
    ("vuln", "缺乏输入验证", "系统未验证输入来源和可信度就纳入执行上下文"),
    ("vuln", "依赖未锁定", "依赖版本未固定、未验证来源完整性"),
    ("vuln", "权限过宽（通配符）", "工具授权范围远超实际需要"),
    ("vuln", "缺乏代码审查", "未对 skill 中可执行代码做安全审查"),
    ("vuln", "缺乏输出过滤", "未对模型/工具输出做安全过滤和消毒"),
    ("vuln", "状态未隔离", "用户或会话间的状态未隔离导致交叉污染"),
    ("vuln", "信任外部内容", "无差别信任从外部源引用的数据或代码"),
    ("vuln", "未声明能力与实际不符", "skill 声明的功能与实际行为不一致"),
]
